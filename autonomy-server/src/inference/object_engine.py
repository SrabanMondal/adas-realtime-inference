...
class ObjectInferenceEngine:
    def __init__(self, yolo26_path, device):
        self.core = ov.Core()
        # Load YOLO26 for INT8 Object Detection
        self.yolo26 = self.core.compile_model(self.core.read_model(yolo26_path), device)
        self.yolo26_request = self.yolo26.create_infer_request()

    def get_perception(self, frame):
        blob = cv2.resize(frame, (640, 640)).transpose(2, 0, 1)[np.newaxis, ...] / 255.0
        results = self.yolo26_request.infer({0: blob})
        detections = results[self.yolo26.output(0)][0] # [x1, y1, x2, y2, conf, cls]
        return detections
